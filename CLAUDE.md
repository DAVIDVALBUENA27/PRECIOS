...
### 2026-07-15: Dev server en WSL sobre /mnt/c no recarga cambios
- **Error**: Con el proyecto en `/mnt/c/...` (drive de Windows montado en WSL), el file-watching de Turbopack no detecta ediciones: el dev server sigue sirviendo páginas viejas, y `.next/dev/types/validator.ts` puede quedar corrupto por escrituras a medias (rompe `tsc --noEmit` con errores que no son del código propio).
- **Fix**: Tras editar archivos, reiniciar el dev server para ver los cambios. Si `tsc` falla dentro de `.next/`, o el server tira errores raros de runtime tipo `adapterFn is not a function` / 404 en rutas que existen, parar el server, `rm -rf .next` completo y reiniciar (borrar solo una parte de `.next` con el server corriendo lo deja inconsistente).
- **Aplicar en**: Cualquier proyecto en este equipo bajo `/mnt/c`. No perseguir "bugs" que solo son el server sirviendo código viejo.
